...
Under the \(48/96\)-block, agentic-aware verifier it scores \(74.321\), compared with
\(97.074\) for the reporting-only oracle and \(70.362\) for TinyLFU-LRU.  Its capacity
token-hit rates are \(0.644\) and \(0.646\), respectively.  The new
...
\begin{equation}
  ...
  \max\left(0,1-\frac{\text{mean-occupancy}_i}{\text{capacity}_i}\right).
\end{equation}

This distinguishes a selective policy that leaves reusable cache space idle from
natural underfill on short or low-reuse workloads where every miss is admitted.
...
current verifier's \(74.321\) score because the capacity sweep, workload panel, and
score terms were subsequently strengthened.
...
Pressure-aware incumbent & 76.630 & 0.674 & 0.687 & 0.521 & 0.421 & 7.944 & 0.065 & 0.067 & 148.7 \\
...
The candidate is the strongest deployable policy by \(6.268\) combined-score points
over TinyLFU-LRU. Its recurrence-gated depth relief closes most of the prior agentic
gap while preserving much lower churn than replacement-style baselines:

\begin{itemize}
  \item \identifier{agentic_tool_workflows} training token hit: \(0.478\) candidate
...
The candidate's ranking benefits from pressure-aware churn control, higher token hit,
and lower source complexity. Recurrence evidence relaxes depth rejection only after
observed repeated access, so first-time deep suffixes still pay the full penalty. Its
score breakdown is:
...
\midrule
Mean workload score & \(+75.044\) \\
Minimum-workload contribution & \(+20.251\) \\
Churn cost & \(-2.230\) \\
Underfill cost & \(-0.804\) \\
Fairness cost & \(-7.663\) \\
Complexity cost & \(-7.968\) \\
\midrule
Combined score & \(\mathbf{76.630}\) \\
\bottomrule
\end{tabular}
\end{center}
...
pressure-aware candidate's token hit rates are \(0.376\) on
...
first value while holding agent-trace churn to \(57.3\) per thousand requests.
Stochastic serving mixes now remain the clearest weakness.

\subsection{Pressure-aware incumbent promotion}
...
Current composed incumbent & 76.630 & 84.598 & 75.044 & 20.251 & 2.230 & 0.804 & 609 &
...
Unlike prior generated candidates, this policy clears final adjudication: selection,
aggregate probe, both recurrence-heavy probe families, and hidden score all improve.
The original policy's main operational gain was a churn-cost reduction from \(5.925\)
to \(2.464\). The recurrence-gated refinement sharply improves agent traces, and the
composed persistent-pressure throttle then lowers churn cost further to \(2.230\)
without giving back that gain. The latest run's data-driven initialization retained
only duplicate valid seed variants, so the archive never reached the three occupied
cells required to activate punctuated equilibrium. Future work should preserve the
current policy while repairing that search-diversity failure.
...
new retained principles.  In particular, the June 7 current-verifier 30-evaluation
...
\begin{enumerate}
  ...
  \item \textbf{Complexity must be charged from exact candidate source and must remain
  unbounded.}  Opaque callable evaluation cannot distinguish a concise formula from a
  large defensive implementation.  Source-based, concave, unbounded complexity
  correctly breaks raw near-ties and discourages dead machinery without imposing a
  brittle hard cap.
  ...
  still pay its full cost.  The latest structured searches confirm that primitive
...
\begin{itemize}
  \item 158 standalone repository tests pass;
  \item \code{ruff check .} passes;
  \item TeX Live compilation succeeds;
  \item \code{git diff --check} is clean;
  \item all 27 workload families build;
  \item YAML/default workload and score parity is confirmed;
  \item the full baseline panel preserves credible ordering: oracle first,
  pressure-aware candidate second, TinyLFU-LRU third, the reporting heuristic fourth,
  and prefix-anchor and vLLM APC following.
\end{itemize}
...
stronger than the credibility baselines even after the verifier is tightened.
...
rate to near the replacement baselines while retaining low churn; it is now the
recommended incumbent. Stochastic serving mixes remain its clearest weakness, while
the compact and structured seeds remain useful alternate parents for deletion-oriented
search.

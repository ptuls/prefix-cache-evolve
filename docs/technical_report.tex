...
\(63.548\).  It also passes the held-out multi-metric agentic surrogate gate, scores
...
post-search agentic gate & A fail-closed seven-metric surrogate-to-probe gate checks
aggregate hit, request-tail hit, worst-quarter hit, admission waste, underfill,
short-reuse eviction misses, and normalized churn.  The held-out probe remains
...
The gate's absolute-gap limits are \(0.12\) for aggregate token hit, \(0.15\) for
request-tail hit, \(0.20\) for worst-quarter hit and admission waste, \(0.16\) for
underfill, and \(0.10\) for short-reuse misses after eviction.  Churn uses
\(\lvert s-p\rvert/\max(\lvert s\rvert,\lvert p\rvert,100)\leq 0.75\).  These initial
limits are a conservative drift alarm calibrated to preserve accepted historical runs,
not a search objective or a claim that the thresholds are optimal.
...
ahead of TinyLFU-LRU at \(63.548\).  It passes all seven checks in the held-out
multi-metric agentic surrogate gate, scores \(3.064\) on the hidden panel versus
\(-2.789\) for TinyLFU-LRU, and retains substantially lower churn.  The result
supports two-stage search: measured over-cap exploration can expose useful structure,
but a separate simplification phase and fail-closed adjudication are required before
promotion.
...
\begin{itemize}
  ...
  \filepath{agentic_surrogate_probe_gate.md}: fail-closed multi-metric comparison of
  agentic surrogate and held-out-probe aggregate, tail, cache-economics, and churn
  behavior;
...
\begin{tikzpicture}[
  ...
  \node[audit] (deployability) at (19.5,-5.45) {
    \textbf{Promotion constraint}\\
    \(\pi\in\Pi_\kappa,\ \operatorname{cx}(\pi)\le650\)\\
    behavioral headroom \(\neq\) deployable winner};
...
measures realized trajectory effects, and the complexity gate determines whether a
behavioral improvement is deployable.}

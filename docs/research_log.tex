...
\begin{itemize}
  ...
  \item The weak-seed rediscovery winner exposed a verifier blind spot: the historical
  token-hit-only agentic tripwire passed with a \(0.1191\) gap, while surrogate and
  held-out-probe wasted-admission rates diverged by \(0.2807\).  The current
  fail-closed surrogate gate therefore checks seven aggregate, tail, cache-economics,
  and churn metrics.  Replaying it preserves the previously accepted production and
  incumbent runs while flagging the rediscovery winner on admission-waste divergence.
\end{itemize}

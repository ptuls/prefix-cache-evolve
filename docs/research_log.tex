...
\newcommand{\blocktag}[1]{\textbf{\color{navy}[#1-token blocks]}}
\newcommand{\mixedblocktag}{\textbf{\color{navy}[mixed block sizes]}}
...
report; this log is the canonical narrative record for experiments, while generated
machine-readable evidence remains under \filepath{artifacts/} and
\filepath{docs/results/}. Historical scores below are not necessarily comparable
across verifier revisions.  Verifier
\identifier{1.0.0} is the first machine-enforced score contract; earlier unstamped
measurements are legacy records rather than \identifier{1.0.0} results.
\end{abstract}
...
Current configurations, manifests, score records, and generated report rows carry a
strict semantic verifier version.  Report code refuses to tabulate missing or mixed
versions.  No semantic version is retroactively assigned to historical runs whose
saved score records predate this contract.

\important{Physical block-size key.}  Score-bearing sections are tagged
\blocktag{8}, \blocktag{16}, or \mixedblocktag.  These tags identify the physical KV
block size used by the simulator, not the workload token granularity.  The latter is
8 tokens in the current 16-token configuration as well.  Scores carrying different
physical-block tags are not directly comparable unless a geometry sweep explicitly
evaluates both sizes under one panel.

\section{Mutation-Operator Productivity}
\label{sec:operator-productivity}

\blocktag{16} The completed 500-budget function-only eviction-specialist run provides
an unusually clear diagnostic of Levi's punctuated-equilibrium design.  The final
snapshot of run
...
All scores use 16-token physical blocks. ``New best'' means a strict improvement in
best score so far.}
...
\blocktag{8} The following run scores were produced with 8-token physical blocks under
the verifier version active at the time.  They are useful for search chronology but
are \risk{not directly comparable} to the
...
\toprule
\textbf{Run ID} & \textbf{Evals} & \textbf{Score (8 tok.)} & \textbf{Runtime} &
...
\blocktag{8} The table in this subsection uses the retained \(8\)-token discovery
verifier.  It documents the environment that produced the promoted policy; it is not
the operative production-oriented ranking.  Under the current \(16\)-token default,
TinyLFU-LRU leads the incumbent by \(0.791\) combined-score points, as summarized in
the workload-model section and the geometry discussion later in this log.
...
\toprule
\textbf{Policy} & \textbf{Score (8 tok.)} & \textbf{Cap. 48} & \textbf{Cap. 96} &
...
\blocktag{16} The shared-capacity robustness panel evaluates all existing policies on
...
separate prefix/decode budgets.  The complete machine-readable panel is retained in
\filepath{artifacts/prefix_kv_cache_reasoning_kv_analysis.json}.

\subsection{Held-out structure-generalization probe}

\blocktag{8} The recurrence-heavy probe is reported but excluded from selection.  The
promoted priority-aware pressure incumbent's token hit rates are \(0.376\) on
...
\blocktag{8} A nominal 300-evaluation search from the compact seed completed 298 recorded
...
\toprule
\textbf{Policy} & \textbf{Selection (8 tok.)} & \textbf{Raw} & \textbf{Mean} &
...
in this subsection; the policy is promoted as the stable incumbent.
...
\blocktag{16} The promoted incumbent has effective complexity 636, leaving only
fourteen nodes below the original 650-node rejection threshold.  A diagnostic of six retained paradigm
...
\filepath{artifacts/prefix_kv_cache_eviction_analysis.json}.

\subsection{Post-expansion evolution result}
\label{sec:post-expansion}

\blocktag{8} Run \filepath{artifacts/prefix_kv_cache_runs/20260606T000036Z} used the
expanded observable surface, canonical multi-timescale primitive, form-aware complexity, and
...
\textbf{Effective cx} & \textbf{Cx cost} & \textbf{Combined (8 tok.)} \\
\midrule
...
The saved winner is byte-identical to the retained compact incumbent; validation, probe, and
...
\blocktag{8} The next iteration converted the first expansion run's hand-rolled fast,
slow, priority, and miss-decay dictionaries into a bounded canonical
...
\begin{tabular}{lrrrrrrrr}
\toprule
\textbf{Policy} & \textbf{Combined (8 tok.)} & \textbf{Raw before cx} &
...
\toprule
\textbf{Run ID} & \textbf{Evals} & \textbf{Cost} & \textbf{Generated combined (8 tok.)} &
...
the compact incumbent unchanged and treated the structured seed as a useful alternate
...
\blocktag{8} A post-verifier-revision hidden diagnostic produced:
...
\section{Admission--Eviction Mechanism Diagnostics}
\label{sec:mechanism-diagnostics}

This section preserves four theory-grounded admission--eviction diagnostics.  Exact
per-decision, per-group, and crossed-policy records remain in the corresponding JSON
files under \filepath{artifacts/}.  Unlike the historical score chronology, these
records carry the machine-enforced verifier identity used when they were generated.

\subsection{Local same-state regret audit}

\blocktag{16} The first falsification test decomposed local regret into avoidable
admission, avoidable rejection, and value-weighted avoidable eviction.  The
future-value surrogate is block token count times remaining request occurrences.
For an incoming block, the audit compares that value with the cheapest legal
displacement; for eviction, it compares the chosen victim with the cheapest legal
victim.  This is a same-state diagnostic rather than a realized counterfactual replay.

The strict claim that admission-side regret dominates in every
workload--capacity--seed group is falsified.  Admission dominates 81 of 123 regretful
groups, leaving 42 eviction-dominant counterexamples.  It nevertheless contributes
\(181{,}327\) future-token units versus \(31{,}565\) for eviction, or \(85.2\%\) of
aggregate surrogate regret.  Admission regret per decision is \(8.56\), compared with
\(5.84\) for eviction; after normalizing within each group by the number of decisions,
admission dominates only 61 of 123 groups.
...
\toprule
Split & Regretful & Admission dominant & Rate & Admission regret &
Eviction regret & Margin \\
\midrule
Hidden & 45 & 31 & 68.9\% & 30,182 & 18,255 & +11,927 \\
Probe & 9 & 6 & 66.7\% & 49,490 & 822 & +48,668 \\
Train & 24 & 12 & 50.0\% & 83,380 & 5,746 & +77,634 \\
Validation & 45 & 32 & 71.1\% & 18,275 & 6,742 & +11,533 \\
\bottomrule
\end{tabular}
\caption{Local admission--eviction regret by split.}
\label{tab:local-regret-by-split}
\end{table}

The strongest recurring counterexamples are shifted heavy-tailed prefixes, shifted
tenant cycles, shared-system-prompt traffic, priority one-off noise, and cyclic
working-set pressure.  They justify an eviction-specific follow-up, but not a uniform
eviction-first rule.  The complete group records and exact counterexample ranking are
retained in
\filepath{artifacts/prefix_kv_cache_admission_eviction_regret_audit.json}.

\subsection{Admission-policy by eviction-policy matrix}

\blocktag{16} A second experiment crossed every distinct admission implementation with
LRU, LFU, cost-aware LRU, the incumbent's compact value-aware eviction, and a
reporting-only constrained next-use control.  Eviction strategy materially changes
realized hit rate under every nontrivial admission policy, but a complex eviction
algorithm is not required: LFU and the incumbent's value-aware rule are usually close.
...
\begin{tabular}{lrrrrrr}
\toprule
Admission & LRU hit & Best deployable eviction & Best hit & Gain &
Oracle hit & Oracle headroom \\
\midrule
Admit all & 0.5205 & Incumbent value-aware & 0.5431 & +0.0226 & 0.5642 & +0.0211 \\
Full blocks only & 0.5123 & LFU & 0.5219 & +0.0096 & 0.5283 & +0.0064 \\
TinyLFU & 0.5286 & Incumbent value-aware & 0.5431 & +0.0145 & 0.5619 & +0.0188 \\
Compact seed & 0.5247 & Incumbent value-aware & 0.5463 & +0.0217 & 0.5666 & +0.0203 \\
Structured seed & 0.5422 & LFU & 0.5558 & +0.0136 & 0.5673 & +0.0114 \\
Structured recurrence & 0.5380 & LFU & 0.5549 & +0.0169 & 0.5694 & +0.0145 \\
Pressure-aware incumbent & 0.5448 & Incumbent value-aware & 0.5535 & +0.0087 &
0.5684 & +0.0150 \\
Oracle future reuse & 0.5677 & Incumbent value-aware & 0.5714 & +0.0037 &
0.5795 & +0.0082 \\
\bottomrule
\end{tabular}
}
\caption{Best eviction rule for each admission implementation.}
\label{tab:admission-eviction-matrix}
\end{table}

The best deployable pairing is structured admission plus LFU at \(0.5558\) mean
token hit.  For the pressure-aware incumbent, value-aware eviction beats LRU in 76
groups, ties in 80, and loses in 6, for a mean \(+0.0087\) token-hit delta.
Constrained next-use reaches \(0.5684\), leaving measurable eviction headroom.  The
result explains why the best replacement rules remain simple: admission determines
which value distribution enters the cache, while a compact frequency-aware victim
ranking captures most of the remaining deployable gain.  The complete 45-cell matrix
is retained in
\filepath{artifacts/prefix_kv_cache_admission_eviction_policy_matrix.json}.

\subsection{Shadow-price tracking and the bandwidth hypothesis}

\mixedblocktag The shadow-price replay covers 60 validation
workload--capacity--seed groups with 96 requests per workload, seeds 11, 23, and 37,
and capacities 8, 16, 24, and 48 blocks.  It defines the oracle water level as
\(\lambda_t^\star=\mu_t/w\), where \(\mu_t\) is the cheapest legal displacement value
and \(w\) is one physical cache slot in token units.  Because the policy score has
arbitrary heuristic units, each replay fits a nonnegative no-intercept scale
\(\beta\) from score to oracle surplus density.  The policy-implied water level is
\(F_t/w-\beta s_t\), preserving the policy's actual zero crossing.

The strict bandwidth prediction is falsified.  Of 22 regretful groups with measurable
oracle-price movement, 16 (\(72.7\%\)) place a disproportionate share of admission
regret in their fastest-change decisions.  Those decisions are only \(3.5\%\) of the
total but contain \(12.5\%\) of admission regret, a \(3.96\times\) fast/slow
regret-density lift.  Mean calibrated tracking RMSE is \(5.9622\) future-value units
per cache token, mean absolute error is \(2.5956\), and mean
shadow-movement/regret correlation is \(+0.184\).
...
\begin{tabular}{lrrrrrrrr}
\toprule
Workload & Regretful & Concentrated & Rate & Fast decisions &
Fast regret & Lift & RMSE & Corr. \\
\midrule
Hotset/cold scan & 3 & 3 & 100.0\% & 4.0\% & 100.0\% & \(\infty\) & 9.0975 & +0.700 \\
Phase shifts & 0 & 0 & 0.0\% & 0.0\% & 0.0\% & \(1.00\times\) & 0.0000 & +0.000 \\
Priority bursts & 6 & 4 & 66.7\% & 5.8\% & 11.0\% & \(2.03\times\) & 7.1514 & +0.057 \\
Stochastic mix & 4 & 2 & 50.0\% & 3.2\% & 9.4\% & \(3.10\times\) & 5.0478 & +0.048 \\
Tenant cycles & 9 & 7 & 77.8\% & 2.6\% & 11.7\% & \(4.99\times\) & 4.5308 & +0.158 \\
\bottomrule
\end{tabular}
}
\caption{Shadow-price bandwidth diagnostic by workload.}
\label{tab:shadow-price-by-workload}
\end{table}
...
\toprule
Capacity & Regretful & Concentrated & Rate & Fast decisions & Fast regret & Lift \\
\midrule
8 & 12 & 10 & 83.3\% & 5.4\% & 14.5\% & \(2.96\times\) \\
16 & 7 & 4 & 57.1\% & 1.8\% & 7.0\% & \(4.18\times\) \\
24 & 3 & 2 & 66.7\% & 1.3\% & 5.1\% & \(4.22\times\) \\
48 & 0 & 0 & 0.0\% & 0.0\% & 0.0\% & \(1.00\times\) \\
\bottomrule
\end{tabular}
\caption{Shadow-price bandwidth diagnostic by cache capacity.}
\label{tab:shadow-price-by-capacity}
\end{table}

The main limitation is identifiability, not merely imperfect tracking.  In 38 of 60
groups, a zero-future-value legal victim pins \(\mu_t\) and therefore
\(\lambda_t^\star\) at zero.  All 48-block groups and all phase-shift-prompt groups
have this degeneracy.  A zero-movement group is not evidence that the policy tracks the
water level successfully; it means that the local relaxation provides no bandwidth
signal.  Pressure half-life is therefore a plausible loop-bandwidth parameter only in
capacity-constrained regimes where displacement price actually moves.  Tracking error
is useful as a quarantined mechanism-diversity coordinate, not as a direct selection
reward or deployable future-value estimate.

\subsection{Crossed causal admission--eviction replay}

\mixedblocktag The decisive component experiment replays every
workload--capacity--seed group in four cells: incumbent admission plus incumbent
eviction (\(II\)), oracle admission plus incumbent eviction (\(OI\)), incumbent
admission plus oracle eviction (\(IO\)), and both oracle components (\(OO\)).  Oracle
admission accepts exactly when future occurrence value exceeds the cheapest legal
displacement value.  Oracle eviction selects the legal leaf with minimum future
occurrence value, breaking ties by furthest next use.  Incumbent score functions still
execute so each intervention replaces only the selected action rule.

The primary outcome is the evaluator's behavioral group score with the future-aware
avoidable-eviction penalty and common complexity charge omitted.  This prevents the
causal outcome from directly rewarding agreement with the audit surrogate.  The full
panel contains 162 groups with 96 requests per workload, capacities 24 and 48 blocks,
and three seeds.
...
\(\sum\tau_{AE}\) & Eviction after \(O_A\) & Retained \\
\midrule
Full panel & 162 & 1704.963 & 518.431 & -258.918 & 259.513 & 50.1\% \\
Audit eviction-dominant & 29 & 530.681 & 419.871 & -340.599 & 79.271 & 18.9\% \\
\bottomrule
\end{tabular}
\caption{Crossed causal component effects.  Eviction after \(O_A\) is
\(Y_{OO}-Y_{OI}=\tau_E+\tau_{AE}\).}
\label{tab:research-causal-components}
\end{table}

Admission has \(3.29\times\) the aggregate main effect of eviction and is larger in
136 of 162 groups.  In the 29 groups labeled eviction-dominant by the local same-state
audit, admission is causally larger in 23.  Oracle admission removes \(81.1\%\) of
their aggregate eviction value, and 18 of 29 retain no positive eviction effect after
admission is repaired.  Across the full panel, 37 of 162 groups retain positive
residual eviction value.
...
\begin{tabular}{lrrrrr}
\toprule
Split & Groups & \(\tau_A\) & \(\tau_E\) & \(\tau_{AE}\) &
Eviction after \(O_A\) \\
\midrule
Hidden & 60 & 445.852 & 110.284 & 40.501 & 150.786 \\
Probe & 12 & 86.019 & 24.103 & -2.933 & 21.170 \\
Train & 30 & 588.057 & 233.135 & -180.684 & 52.451 \\
Validation & 60 & 585.036 & 150.908 & -115.802 & 35.106 \\
\bottomrule
\end{tabular}
\caption{Causal component effects by split.}
\label{tab:research-causal-by-split}
\end{table}

The interaction adjudicates the hybrid question directly.  Admission repairs dissolve
most apparent eviction value in the audit-labeled counterexamples, so those local
surrogate labels should not be read as a mandate for globally richer eviction.
Residual eviction value is nevertheless real and concentrated: hidden
heavy-tailed-prefix, hidden cyclic-pressure, agentic-workflow, and tenant-cycle
families retain material gains after oracle admission.  The engineering conclusion is
asymmetric: optimize admission first, then gate richer eviction logic into the
minority regimes where its value survives the admission intervention.  Secondary
outcomes in the JSON artifact---token hit, block hit, saved tokens, p95 latency, and
churn---support the same directional conclusion.

\subsection{Constructive shadow-price test: switching-cost dual policies}
\label{sec:switching-cost-dual}

\blocktag{16} The bandwidth diagnostic above is observational.  A constructive
follow-up asks whether a policy built to track the oracle water level directly beats
the incumbent's slow congestion signal.  Three prototype policies under
\filepath{experiments/} keep the incumbent's value features but replace its pressure
state with an explicit scalar dual \(\lambda\).  A single retention index
\(V_b=\log(1{+}f_b)+0.2\log(1{+}r_b)+0.28\,p_b+0.35\log(1{+}d_b)+1.2\log(1{+}c_b/96)\)
scores both actions from decayed observed frequency \(f_b\), recency \(r_b\), and
priority evidence \(p_b\) with structural descendant count \(d_b\) and recompute cost
\(c_b\).  Eviction removes \(\arg\min_b V_b\); each eviction round contributes its
cheapest candidate \(\hat\mu=\min_b V_b\) as a sample of the marginal displacement
price, and the dual tracks it as \(\lambda \leftarrow \lambda+0.5(\hat\mu-\lambda)\)
with a between-request relaxation \(\lambda\leftarrow 0.97\lambda\) floored at
\(0.6\,\text{pressure}_t\).  Admission accepts when \(V_b-\lambda-m>0\) for a
switching margin \(m\).

The undamped variant (\(m=0\)) confirms both the mechanism and its cost.  It tracks
the oracle price far better than the incumbent---fast/slow regret-density lift rises
from \(0.35\times\) to \(12.13\times\) and shadow-movement/regret correlation from
\(+0.008\) to \(+0.313\)---yet its charged operative score is only \(58.776\) because
it churns roughly five times as much at an unchanged token hit rate.  A dead-band on
the dual update plus a recent-eviction guard leaves churn essentially unchanged: the
churn is not admit/evict/re-admit thrash of individual blocks.  It follows instead
from marginal-swap admission---with \(\lambda\) pinned to the cheapest resident, any
block worth marginally more is admitted---so restoring a switching margin \(m\), the
same barrier the incumbent's constant \(-0.7\) offset provides, is the correct
control.
...
\begin{tabular}{lrrrrrr}
\toprule
Policy & Charged & Churn/1k & Token hit & Nodes & Lift & Corr. \\
\midrule
Pressure-aware incumbent & 65.649 & 163.9 & 0.596 & 572 & \(0.35\times\) & \(+0.008\) \\
Explicit dual (\(m{=}0\)) & 58.776 & 858.3 & 0.599 & 364 & \(12.13\times\) & \(+0.313\) \\
Dead-band \(+\) guard & 57.618 & 859.4 & 0.598 & 470 & \(12.92\times\) & \(+0.301\) \\
Switching cost (\(m{=}0.7\)) & 63.219 & 281.0 & 0.607 & 372 & \(1.40\times\) & \(+0.088\) \\
\bottomrule
\end{tabular}
\caption{Explicit-dual mechanism variants on the operative 16-token validation panel.
Lift is the fast/slow admission-regret-density ratio and Corr.\ is the
shadow-movement/regret correlation from the same audit.}
\label{tab:switching-cost-variants}
\end{table}

\good{A one-parameter sweep of the switching margin clears the incumbent.}  Evaluated
in process on the operative validation panel, the production incumbent reproduces its
raw \(73.252\) and charged \(65.649\) scores exactly as a control, and the
\(m=0.7\) point reproduces the full-run charged \(63.219\).  The variant peaks above
the incumbent: charged \(66.145\) at \(m=0.9\) with a \emph{higher} token hit rate,
and \(67.333\) at \(m=1.1\) at roughly half the incumbent's churn, using \(372\)
charged nodes against \(572\).
...
\begin{tabular}{rrrrr}
\toprule
Margin \(m\) & Raw & Charged & Churn/1k & Token hit \\
\midrule
0.40 & 67.858 & 62.352 & 638.4 & 0.599 \\
0.60 & 70.047 & 64.542 & 361.0 & 0.606 \\
0.80 & 69.790 & 64.284 & 187.2 & 0.609 \\
0.90 & 71.651 & 66.145 & 136.7 & 0.611 \\
1.00 & 71.366 & 65.860 & 121.8 & 0.609 \\
1.10 & 72.838 & 67.333 & 92.7 & 0.589 \\
1.20 & 67.778 & 62.272 & 61.6 & 0.583 \\
\bottomrule
\end{tabular}
\caption{Switching-margin sweep for the explicit-dual policy.  Charged score is the
raw combined score minus the fixed \(5.506\)-point source-complexity charge.}
\label{tab:switching-cost-sweep}
\end{table}

The result reframes the bandwidth hypothesis.  Tracking fidelity and churn trade off
directly, and the charged score follows churn, not tracking: the incumbent occupies
the low-tracking, low-churn corner the verifier rewards, and the switching cost is the
principled churn lever.  The operative object is therefore a restless-bandit index
\emph{with} a switching penalty rather than a plain Whittle or Gittins index.
\risk{These are operative-validation combined scores only.}  The variant has not
cleared probe tripwires, the hidden panel, or cross-family adjudication; the
\(m=1.1\) peak is narrow (\(m=1.0\) scores \(65.860\), \(m=1.2\) scores \(62.272\));
and every coefficient other than \(m\) is a first guess.  It is nonetheless a simpler,
lower-churn, higher-scoring policy than the prior incumbent.

\good{The \(m=0.9\) variant was promoted to the current production incumbent}
(\identifier{production_dual_16tok_20260706}).  It cleared the full gate battery
against the prior incumbent: validation \(66.145\) versus \(65.649\), held-out probe
\(77.177\) versus \(74.899\), and held-out hidden \(4.042\) versus \(3.064\) (best
deployable policy on the hidden panel), at \(372\) effective nodes versus \(572\) and
lower churn.  Because the margin was selected on validation while probe and hidden
were not, the held-out gains are generalization rather than selection overfit.  The
score-weight sensitivity sweep is robust to increasing churn, underfill,
wasted-admission, and avoidable-eviction weights, but \risk{TinyLFU-LRU overtakes it
once the fairness weight reaches \(1.5\times\) nominal}.  The margins are modest and
only the switching margin is tuned, so the honest reading is ``at least as good as the
prior incumbent, and plausibly better, while simpler and lower-churn.''

\subsection{The tracking--churn frontier}
\label{sec:tracking-churn-frontier}

Sweeping the switching margin \(m\) of \S\ref{sec:switching-cost-dual} while auditing
shadow-price tracking at each setting exposes the tradeoff structure the verifier is
actually navigating.  As \(m\) rises, both the fast/slow admission-regret-density lift
and churn fall monotonically: high-fidelity tracking and low churn cannot be held at
once.
...
\begin{tabular}{lrrrrr}
\toprule
Policy & Lift & Corr. & Churn/1k & Token hit & Charged \\
\midrule
Explicit dual (\(m{=}0\)) & \(12.13\times\) & \(+0.313\) & 858.3 & 0.599 & 58.776 \\
Switching cost (\(m{=}0.4\)) & \(6.96\times\) & \(+0.185\) & 638.4 & 0.599 & 62.352 \\
Switching cost (\(m{=}0.7\)) & \(1.40\times\) & \(+0.088\) & 281.0 & 0.607 & 63.219 \\
Switching cost (\(m{=}0.9\)) & \(0.87\times\) & \(+0.085\) & 136.7 & 0.611 & 66.145 \\
Switching cost (\(m{=}1.2\)) & \(0.52\times\) & \(+0.006\) & 61.6 & 0.583 & 62.272 \\
Evolved incumbent & \(0.35\times\) & \(+0.008\) & 163.9 & 0.596 & 65.649 \\
\bottomrule
\end{tabular}
\caption{Tracking fidelity, churn, hit rate, and charged score across the switching
margin.  Lift is the fast/slow admission-regret-density ratio; higher means better
oracle-price tracking.  The evolved incumbent is a different policy, shown for
comparison, not a point on the margin curve.}
\label{tab:tracking-churn-frontier}
\end{table}
...
\begin{tikzpicture}[
  ...
  frontierpt/.style={circle,fill=navy,inner sep=1.7pt},
  dompt/.style={circle,draw=gray,fill=white,line width=0.7pt,inner sep=1.7pt},
  promo/.style={star,star points=5,star point ratio=2.2,fill=navy,draw=navy,inner sep=1.6pt},
]
% x = (log10(churn)-1.7)*8 (log churn axis); y = (hit-0.58)/0.005 (token hit rate)
\draw[->,thick] (0,0) -- (10.9,0) node[below right] {churn / 1{,}000 requests (log)};
\draw[->,thick] (0,0) -- (0,7.3) node[above left] {validation token hit rate};
\foreach \x/\lab in {0.63/60,2.4/100,4.81/200,7.22/400,9.62/800} {\draw (\x,0) -- (\x,-0.09) node[below] {\lab};}
\foreach \y/\lab in {0/0.580,2/0.590,4/0.600,6/0.610} {\draw (0,\y) -- (-0.09,\y) node[left] {\lab};}
% efficient frontier (minimize churn, maximize hit): m=1.2 -> 1.1 -> 1.0 -> 0.9
\draw[navy,very thick] (0.74,0.6) -- (2.15,1.8) -- (3.09,5.8) -- (3.49,6.2);
% frontier points
\node[frontierpt,label={[lab]below right:$m{=}1.2$ ($0.5\times$)}] at (0.74,0.6) {};
\node[frontierpt,label={[lab]right:$m{=}1.1$}] at (2.15,1.8) {};
\node[frontierpt,label={[lab]left:$m{=}1.0$}] at (3.09,5.8) {};
\node[promo,label={[lab,navy]above:$m{=}0.9$ promoted ($0.9\times$)}] at (3.49,6.2) {};
% dominated points (higher tracking, to the lower-right of the frontier)
\node[dompt,label={[lab]above right:$m{=}0.7$ ($1.4\times$)}] at (5.99,5.4) {};
\node[dompt,label={[lab]below:evolved ($0.35\times$)}] at (4.12,3.2) {};
\node[dompt,label={[lab]above:$m{=}0.4$ ($7.0\times$)}] at (8.84,3.8) {};
\node[dompt,label={[lab]below:$m{=}0$ ($12.1\times$)}] at (9.87,3.8) {};
% annotations
\node[navy,lab,anchor=west,align=left] at (4.3,0.9) {efficient frontier (low tracking)};
\draw[navy,-{Stealth[length=5pt]}] (4.2,1.05) -- (2.75,3.4);
\node[gray,lab,anchor=west,align=left] at (5.9,6.7) {dominated points:\\higher tracking, higher churn};
\draw[gray,-{Stealth[length=5pt]}] (7.9,6.5) -- (8.75,4.25);
\draw[gray,-{Stealth[length=5pt]}] (6.15,6.4) -- (6.1,5.7);
\end{tikzpicture}
\caption{The churn--hit-rate Pareto plot across the switching margin.  Upper-left is
better (lower churn, higher hit rate).  The efficient frontier (navy) is the
low-churn, low-tracking arm; the promoted \(m=0.9\) policy is its peak.  The
high-tracking policies (larger lift) sit to the lower-right and are Pareto-dominated.
Marker lift is the fast/slow admission-regret-density ratio.}
\label{fig:tracking-churn-frontier}
\end{figure}

Two conclusions follow.  First, the verifier's churn weight selects the operating
point: the score-weight sensitivity sweep gives the \(m=0.9\) policy rank 1 at nominal
and \(2\times\) churn weight but rank 3 (behind LFU) when the churn weight is zeroed, so
raising the churn price slides the optimum toward higher \(m\), lower churn, and lower
tracking.  Second, and more sharply, \important{faithful tracking is not on the
efficient frontier---it is Pareto-dominated.}  In hit-rate/churn space the \(m=0.9\)
policy has both a higher token hit rate and lower churn than every higher-tracking point
below it, so those points are strictly worse on both axes.  The efficient frontier is
the \(m\gtrsim0.9\) arm, which trades hit rate for churn while tracking fidelity is
already below \(1\times\).  The charged-score optimum therefore sits in the low-tracking
region regardless of the churn weight.

The mechanism is the slack constraint documented in
\S\ref{sec:switching-cost-dual}: on this panel most scored decisions have a
zero-value legal victim, so the oracle price rarely moves and tracking it earns
nothing while adding reactive churn.  \risk{Tracking would only become valuable---bending
the frontier so high-fidelity policies stop being dominated---under a workload and
verifier where the shadow price moves fast and mistracking measurably costs hit rate.}
That is a benchmark-design lever, not a policy one.  The frontier here is also traced by
a single value-function family varying one parameter, so it is an achievable frontier,
not a proven-optimal one; a better value function could dominate it.

The charged score the verifier actually optimizes is not monotone in churn: plotting it
against churn (Figure~\ref{fig:score-churn}) gives an inverted-U with an interior
optimum near 90--140 evictions per 1{,}000 requests.  Only the low-churn plateau clears
the TinyLFU-LRU baseline; the promoted \(m=0.9\) sits on that plateau rather than at the
marginally higher but narrow \(m=1.1\) spike.
...
% x = (log10(churn)-1.7)*8 (log churn); y = (charged-58)*0.7
\draw[->,thick] (0,0) -- (10.9,0) node[below right] {churn / 1{,}000 requests (log)};
\draw[->,thick] (0,0) -- (0,7.4) node[above left] {charged score};
\foreach \x/\lab in {0.63/60,2.4/100,4.81/200,7.22/400,9.62/800} {\draw (\x,0) -- (\x,-0.09) node[below] {\lab};}
\foreach \y/\lab in {1.4/60,2.8/62,4.2/64,5.6/66,7.0/68} {\draw (0,\y) -- (-0.09,\y) node[left] {\lab};}
% TinyLFU-LRU baseline
\draw[gray,dashed] (0,3.884) -- (10.6,3.884);
\node[gray,lab,anchor=west] at (6.5,4.15) {TinyLFU-LRU ($63.548$)};
% margin sweep trajectory (ordered by churn)
\draw[navy,thick] (0.74,2.990) -- (2.15,6.533) -- (3.09,5.502) -- (3.49,5.702)
  -- (5.99,3.653) -- (8.84,3.046) -- (9.87,0.543);
% points
\node[frontierpt,label={[lab]below:$m{=}1.2$}] at (0.74,2.990) {};
\node[frontierpt,label={[lab]above:$m{=}1.1$ peak}] at (2.15,6.533) {};
\node[frontierpt,label={[lab]below left:$m{=}1.0$}] at (3.09,5.502) {};
\node[promo,label={[lab,navy]above right:$m{=}0.9$ promoted}] at (3.49,5.702) {};
\node[frontierpt,label={[lab]below right:$m{=}0.7$}] at (5.99,3.653) {};
\node[frontierpt,label={[lab]above:$m{=}0.4$}] at (8.84,3.046) {};
\node[frontierpt,label={[lab]above left:$m{=}0$}] at (9.87,0.543) {};
\node[dompt,label={[lab]right:evolved incumbent}] at (4.12,5.354) {};
\end{tikzpicture}
\caption{Charged score versus churn across the switching margin (log churn axis).  The
verifier's objective is an inverted-U in churn: score rises as churn falls, peaks near
the promoted \(m=0.9\) plateau, then collapses once the margin starves admission.  Only
the low-churn plateau clears the TinyLFU-LRU baseline (dashed).  The evolved incumbent
(open circle) is shown for comparison.}
\label{fig:score-churn}
\end{figure}

\subsection{Chronology: from the evolved incumbent to the switching-cost dual}
\label{sec:derivation-chronology}

The switching-cost dual was not discovered by search.  It was derived by a sequence
of diagnostic steps that each answered one question and set up the next.  This
subsection records that path so the reasoning, not only the result, is reproducible.

\begin{enumerate}
  \item \important{Harden the headline statistics.}  The evolved incumbent's lead over
  TinyLFU-LRU was first stated per \((\text{workload},\text{capacity},\text{seed})\)
  cell.  That comparison is pseudo-replicated: 11--12 of 20 family--capacity cells are
  seed-invariant, so nominal trial rows are not independent.  Collapsing to the ten
  designed workload families gives an honest cluster-robust statement---the evolved
  incumbent led in 9 of 10 families---and a descriptive-only interval, because three
  fixed hand-designed families are not a random population sample.
  \item \important{Locate the edge across the field and geometries.}  The incumbent is
  the strongest deployable policy at its geometry but only narrowly ahead of LFU and
  TinyLFU-LRU, and its apparent large-block collapse is an artifact of block-count
  normalization: holding capacity fixed in tokens, it stays ahead at coarser blocks.
  The advantage is concentrated in churn and in capacity-constrained regimes, which
  pointed at admission economics rather than eviction ranking.
  \item \important{Read the policy as a shadow-price index.}  Admission compares a
  per-block value density against a price; eviction removes the minimum-index leaf.
  The admission shadow-price audit (Table~\ref{tab:shadow-price-by-workload}) showed the
  incumbent tracks the oracle water level only as a slow congestion aggregate, not its
  fast per-decision movement---a low-bandwidth controller.
  \item \important{Test the bandwidth hypothesis constructively.}  Building an explicit
  dual calibrated to the marginal evicted victim (\S\ref{sec:switching-cost-dual})
  raised tracking fidelity by an order of magnitude but churned about five times as
  much and scored worse.  Dead-band and re-admission guards did not reduce churn, which
  falsified the thrash explanation and isolated marginal-swap admission as the cause.
  \item \good{Add the switching cost and tune one parameter.}  Restoring an admission
  margin---the barrier the compact policy's constant offset had always supplied---cut
  churn back to the incumbent's range.  A single-parameter margin sweep peaked above the
  incumbent, and the \(m=0.9\) variant cleared the full gate battery on validation and
  both held-out panels.  It was promoted to \identifier{production_dual_16tok_20260706}.
\end{enumerate}

The general lesson is that faithful shadow-price tracking and low churn are in tension,
and this verifier rewards the low-churn corner; the switching cost is the parameter that
selects the operating point along that tradeoff.

\subsection{Conversation-derived transfer test: admission selectivity anti-transfers}
\label{sec:wildchat-transfer}

\blocktag{16} The first external-trace experiment replays a WildChat-derived
trace---2{,}000 conversations converted to 9{,}362 anonymized requests with HMAC
identifiers and no prompt text---through every deployable policy at the operative
16-token, 24/48-block geometry.  The result is a clean negative: the ranking is
ordered \emph{inversely} by admission selectivity.
...
\begin{tabular}{lrrr}
\toprule
Policy & Combined & Token hit & Churn/1k \\
\midrule
vllm\_apc (admit-all, pinned LRU) & 13.055 & 0.240 & 6{,}146.6 \\
lfu / lru / other admit-all & 12.5--12.7 & 0.240 & \(\sim\)6{,}525 \\
tinylfu\_lru (evidence hurdle) & 1.457 & 0.186 & 4{,}818.4 \\
Switching-cost dual incumbent & \(-22.398\) & 0.096 & 2{,}603.7 \\
Prior evolved incumbent (smoke slice) & \(-22.689\) & 0.004 & 73.7 \\
no\_cache & \(-23.414\) & 0.000 & 0.0 \\
\bottomrule
\end{tabular}
\caption{WildChat-derived replay at the operative geometry.  The prior incumbent row is
from the 380-request smoke slice; all others are the full 9{,}362-request replay.}
\label{tab:wildchat-transfer}
\end{table}

The mechanism is a regime mismatch, and the shadow-price frame explains it exactly.
The median prompt is 736 tokens---46 blocks, larger than the entire 24-block
tier---and 65\% of requests exceed that tier.  Each conversation turn's prompt
extends the previous one, so a first-seen block has near-certain immediate reuse if
the conversation continues: the optimal cold-start prior on a new arm is
\emph{optimistic}, where the synthetic panel taught a pessimistic one.  The
perpetually overflowing cache saturates \metric{recent_admission_pressure}, so the
dual's price floor holds the admission hurdle permanently high, and the prior
incumbent's pressure state shuts admission entirely (hit rate 0.004; the promoted
dual transfers strictly better at 0.096).  A supplementary smoke replay at 96/192
blocks does not rescue selectivity: admit-all reaches hit rate 0.58 while the dual
reaches 0.17, so the failure is the admission prior, not capacity alone.  Finally,
the churn penalty saturates at its cap (\metric{churn_cap} 25) at 6{,}500 evictions
per 1{,}000 requests, so scoring in this regime is effectively hit-rate-only; whether
admit-all ``really'' wins depends on whether eviction churn at that volume carries
real cost, which the synthetic weighting was never calibrated to answer.

Three consequences.  First, the headline is now explicitly bounded: the promoted
policy's lead is a synthetic-panel result, and its admission economics anti-transfer
to conversation-shaped traffic at the operative geometry.  Second, the deployable fix
is a \important{session-aware admission prior}---\metric{session_id} is
candidate-visible, and boosting admission for blocks of a currently active session is
precisely the optimistic prior this traffic rewards; it joins the search objectives.
Third, this is the strongest argument yet for a next verifier revision: either a
conversation-growth workload family joins the panel, or trace replay becomes a
reported non-selection gate, because otherwise evolution will keep sharpening an
admission prior that points the wrong way on real conversations.  The replay artifact
and conversion manifest are pinned under \filepath{docs/results/}; the trace itself is
reproducible from the retained HMAC key.
...
There were five distinct thresholds of readiness:
...
\begin{enumerate}
  ...
  loaded from source, executed in a resource-limited worker, checked for time and memory
  ...
  \item \textbf{A promoted incumbent remained auditable after the run.}  Ignored run
  artifacts and mutable filenames were not a durable promotion record: a formatter
  could preserve behavior while changing the exact source hash, and provenance stayed
  scattered across prose and local directories.  Promoted policies are now retained as
  immutable source-and-manifest bundles.  A registry assigns production and discovery
  roles, while CI validates source hashes, effective complexity, import targets,
  benchmark identities, and the originating candidate-artifact hash.
\end{enumerate}

The practical bootstrap path therefore became:

\begin{lstlisting}[style=shell]
.venv/bin/python -m pip install -e '.[dev,evolution]'
.venv/bin/pytest -q
...
shortcut or blind spot.  They remain explicitly unversioned unless their original
artifact records a verifier semantic version.

\subsection{Future-information contract audit}

The retained deployable policies do not use explicit future information.  Source
inspection found future-reuse references only in the two reporting-only oracle
baselines.  Historical candidates that attempted to reference future-reuse fields
could not receive those values during normal evaluation, and the current static gate
rejects such source before execution.

The audit did identify indirect leakage opportunities in the evaluator contract.
Those surfaces were removed so deployable callbacks receive only online policy
signals:
...
\toprule
\textbf{Surface} & \textbf{Previous behavior} & \textbf{Current contract} \\
\midrule
Future reuse and next-use distance &
Available only when a future-aware mode was enabled &
Deployable callbacks receive no values; baselines requiring them cannot be deployable \\
Workload-generation seed &
Passed to the candidate factory &
Factory receives an independent fixed policy seed \\
Request ID &
Sequential benchmark position &
Stable opaque hash \\
Request type &
Descriptive synthetic workload label &
Normalized to \identifier{request} \\
Prompt tokens &
Usually empty but accepted from custom streams &
Always empty in candidate-visible metadata \\
Internal regret state &
Computed by the simulator for reporting &
Never copied into deployable block metadata \\
\bottomrule
\end{tabularx}
\end{center}

Regression tests cover miss, hit, admission, and eviction callbacks, policy-seed
independence, and rejection of future-aware deployable baselines.  This establishes an
experimental information contract, not an adversarial Python security boundary.
Candidate code still executes in an isolated worker with resource limits rather than a
capability-separated policy DSL, so intentionally malicious reflection remains outside
the claim.  The ranked policies do not use those techniques, and the removed metadata
surfaces were not consumed by their source.
...
Run candidate evaluation in a resource-limited worker with wall-clock and CPU limits,
Linux address-space limiting, Python memory monitoring,
...
\blocktag{8} Table~\ref{tab:journey-runs} records the retained discovery-verifier runs
that most directly changed the system.  The earlier June 6 structured experiments used a prior
...
\bottomrule
\end{tabularx}
\caption{Retained discovery-verifier evolution journey using 8-token physical blocks.}
\label{tab:journey-runs}
\end{table}

\begin{figure}[H]
\centering
\input{figures/incumbent_eval_trajectory.tex}
...
global best 8-token score, so a flat segment means that additional evaluations did not
improve the retained policy.  Panel B expands the final targeted run: gray and green points are
...
\blocktag{8} The project also took a deliberate detour through richer state
representations.  The observable surface was expanded with recurrence, subtree, and regime context, and a
...
\blocktag{8} The first clear discovery-verifier improvement was the \(76.069\)
...
\blocktag{16} The production full-policy lane has now crossed the 16-token TinyLFU-LRU
target without hand-authoring the candidate.  Its 300-evaluation search first found a \(656\)-node,
...
analysis, and unretained experiments are not priced.

\begin{center}
\small
\begin{tabular}{lrr}
\toprule
\textbf{Stage} & \textbf{Evaluations} & \textbf{Recorded API cost} \\
\midrule
...
The retained sources are the run summaries under
\filepath{artifacts/prefix_kv_cache_runs},
\filepath{artifacts/prefix_kv_cache_production_runs},
\filepath{artifacts/prefix_kv_cache_simplification_runs}, and
\filepath{artifacts/prefix_kv_cache_eviction_specialist_runs}, plus the function-only
snapshot under \filepath{runs/20260608_150431_280387}.  Early and structured stages
whose run directories are no longer retained are recorded in the chronology tables
above.
...
\mixedblocktag A fixed-block-count geometry sweep then tested both incumbents at 24, 48, 96, and
...
\mixedblocktag A second sweep held cache capacity fixed at 384 and 768 tokens while increasing
...
\mixedblocktag A follow-up increased block capacity to 24, 48, 96, and 128 entries for every coarse
...
\mixedblocktag TinyLFU-LRU was then evaluated on the same four-capacity grid.  The evolved policy
...
\mixedblocktag The benchmark's vLLM APC approximation is weaker throughout the same grid.  The
...
\mixedblocktag The final geometry sweep covered every registered baseline.  Plain LFU narrowly
...
by coarse blocks.  The full score matrix is retained in
...
\blocktag{8} Every score in this table comes from the 8-token discovery or
pre-discovery verifier lineage.
...
adjudication is retained in the tables above.

\subsection{Controlled attempts that failed adjudication}

\begin{itemize}
  \item \blocktag{8} \textbf{Broad explicit recurrence state was harmful.}  Removing recurrence
  ...
  \item \blocktag{8} \textbf{Priority-only decay overfit validation.}  It improved charged
  ...
  \item \blocktag{8} \textbf{Primitive adoption did not solve policy bloat.}  After the prompt and
  ...
  \item \blocktag{8} \textbf{Direct target-family gains were not sufficient.}  Both the
  ...
  \item \blocktag{16} \textbf{The compact eviction-weight graft was a complexity mirage.}  The
  ...
  simplifying other full-policy code, not from better eviction behavior.
\end{itemize}
...
\begin{itemize}
  \item \blocktag{8} A June 7 discovery-verifier 30-evaluation snapshot produced elites with
  ...
  \item \blocktag{8} Two 98-evaluation searches retained the seed from evaluation 1 and then stayed
  ...
  \item \blocktag{8} The preceding 298-evaluation diversified run occupied one archive cell, reported
  ...
  \item \blocktag{8} The retained June 8 298-evaluation run repaired archive coverage to ten cells and
  ...
  \item \blocktag{16} The completed 500-budget function-only eviction run filled all sixteen archive
  ...
  \item \blocktag{16} A 98-evaluation weak-initial-seed rediscovery pilot assembled a
  pressure-conditioned incumbent-family mechanism and raised charged selection from
  \(53.366\) to \(59.404\), recovering \(49.2\%\) of the charged selection gap and
  \(97.0\%\) of the raw selection gap.  It did not establish discoverability: the
  generated policy reached \(1{,}214\) effective nodes, had negative charged probe-gap
  recovery, recovered only \(45.5\%\) of the hidden charged gap, and failed the
  multi-metric agentic gate on admission-waste divergence.  The retained incumbent
  therefore remains a path-dependent local-evolution result until repeated neutral
  weak-seed runs pass the separate behavioral rediscovery protocol.
  \item \blocktag{16} Three fresh June 14 weak-seed searches made that protocol
  executable but did not support discoverability.  Across \(894\) evaluations,
  \(\$21.5381\), and \(8{,}705\) seconds, zero runs recovered at least \(80\%\) of
  the charged weak-seed-to-incumbent gap on every canonical panel while remaining
  deployable and passing the agentic gate.  The ordinary-score run overfit selection:
  its 459-node generated policy scored \(67.444\) on selection but only \(30.661\)
  on probe and failed the agentic gate.  Ranking by the minimum of ordinary score and
  non-quarantined agentic guidance repaired that gate and raised probe to \(50.272\),
  but selected a 216-node high-churn stateless policy with \(53.975\) selection and
  \(-16.328\) hidden score.  After documenting generic stateful primitives and
  enabling inspiration-based recombination, the final 298-evaluation run retained
  the weak seed as its search winner.  Its strongest generated mutation was 708
  nodes, failed the agentic gate, and scored below the weak seed on selection, probe,
  and hidden panels.  Exact incumbent replay is reproducible; independent weak-seed
  rediscovery is not yet established, and the next search-process target is
  productive stateful mutation and recombination rather than another unchanged
  budget increase.
  \item \blocktag{16} Adding shared prefix/decode capacity initially revealed a verifier bug: total
...
\subsection{Score identity became a machine-enforced invariant}

Historical scores were already labeled by verifier revision in prose, but verifier
semver alone did not distinguish a score-weight change, geometry change, split change,
or regenerated request panel.  Every score-bearing record now carries three fields:
\identifier{verifier\_version}, \identifier{panel\_sha256}, and
\identifier{evaluation\_context\_sha256}.  The panel hash covers the ordered request
streams and panel geometry.  The context hash covers verifier semver, the normalized
evaluator configuration, and the panel hash.

This forced an important reporting distinction.  A baseline table at one geometry must
have one exact score identity and now fails closed on any mismatch.  A geometry sweep
or prefix-only/shared-capacity comparison intentionally spans contexts, so it records
one identity per geometry or mode and validates each comparable slice independently.
The resulting rule is operational rather than narrative: numbers may share a table only
when the report declares whether they are exact-context comparisons or explicitly
indexed cross-context measurements.

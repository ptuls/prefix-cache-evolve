...
\(63.548\).  It passes both held-out per-family tripwires: the agentic gap is
\(0.0918<0.12\), and the cyclic working-set gap is \(0.2198<0.25\).  It also scores
...
The earlier incumbent scored \(62.757\) at the primary 16-token size.  The promoted
...
promotion tripwires & Per-family token-hit divergence limits are 0.12 for
agentic branching and 0.25 for cyclic working-set pressure.  Probe values remain
...
and artifact sources are listed in
\filepath{docs/results/search_cost_tally.md}.
...
ahead of TinyLFU-LRU at \(63.548\).  It passes the agentic tripwire with a \(0.0918\)
gap against a \(0.12\) limit and the cyclic working-set tripwire with a \(0.2198\)
gap against a \(0.25\) limit.  It scores \(3.064\) on the hidden panel versus
\(-2.789\) for TinyLFU-LRU and retains substantially lower churn.  The result supports
two-stage search: measured over-cap exploration can expose useful structure, but a
separate simplification phase and fail-closed adjudication are required before
promotion.  The two required runs cost \$5.070 together; the broader retained lineage
...
results are preserved in \filepath{docs/research_log.tex} and
\filepath{docs/results/}.

\section{Evolution-Discovered Principles}
\label{sec:evolution-principles}

The supplementary research log and generated result summaries contain more information
than the retained winner alone.  This section consolidates that evidence into a
...
\begin{itemize}
  ...
  \filepath{surrogate_probe_tripwires.md}: fail-closed per-family comparisons for
  agentic branching and cyclic working-set pressure; the legacy
  \filepath{agentic_surrogate_probe_tripwire.*} files remain for compatibility;
...
Detailed chronology and rejected solutions are intentionally separated into
\filepath{docs/research_log.tex}; generated measurements remain under
\filepath{docs/results/}.  This keeps the main report focused on the current method,
claims, limitations, and reproducibility contract.
...
compilable \filepath{docs/research_log.tex}.  Generated result summaries remain under
\filepath{docs/results/}, and exact run artifacts remain under \filepath{artifacts/}.
...
composes to \(1{,}011\) nodes.  Guarded age plus descendant protection captures
\(98.0\%\) of the full specialist's raw gain, but still composes to 775 nodes.  A
single deployable distillation--raising descendant protection from \(0.2\) to
\(0.6\)--keeps complexity at 636 and gains \(0.168\) raw selection, but loses
...
The corrected model suggests a hybrid policy family rather than a larger unconditional
ranker.  Let \(h_t\in\{0,1\}\) be an online regime gate estimated from candidate-visible
...
The local audit is not enough to establish causal component value.  The decisive study
is a crossed replay using incumbent and oracle components.  Let
...
\end{align}
The corrected hypothesis is not \(\tau_A(g)>\tau_E(g)\) for every group.  It is that
\(\sum_g\tau_A(g)>\sum_g\tau_E(g)\) may hold in aggregate, while the audit-identified
counterexample families should satisfy \(\tau_E(g)>\tau_A(g)\).  A large interaction
term \(\tau_{AE}\) would show that admission and eviction cannot be optimized
independently because admission changes the future legal-victim distribution.
